...
"""
conf_file='/etc/zt_nss/zt.conf'
pid_file='/var/run/zt.pid'
...
def processMessage(sock,noact, debug):
...
def zt_server(url,headers,noact,timeout,bind,port,debug):
...
def run(pidf,url,headers,noact,timeout,bind,port,debug):
	if debug:
		print('run()')
		dbg_file='/tmp/zt.txt'
	...
	with daemon.DaemonContext(
		...
		zt_server(url,headers,noact,timeout,bind,port,debug)
...
if __name__ == "__main__":
	...
	if args.foreground == True:
		...
		run(args.pid_file, url,headers,noact,args.timeout,args.bind,args.port,args.debug)
	if os.path.exists(sock_file):
		os.remove(sock_file)
	sys.exit(0)
